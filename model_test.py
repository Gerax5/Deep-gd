...
checkpoint_path = "Deep-gd/geometry2_dash_dqn.pth"
...
if "model_state_dict" in checkpoint:
    ...
    print(f"Cargando modelo con {state_dim} entradas (nuevo formato).")
else:
    state_dim = len(GeometryEnv().reset())
    state_dict = checkpoint
    print(f"Cargando modelo antiguo con {state_dim} entradas.")
...
def get_action(state):
    with torch.no_grad():
        qvals = policy(torch.tensor(state, dtype=torch.float32).unsqueeze(0))
...
while running:
    ...
    if done:
        print(f"Game Over | Score final: {int(score)}")
        running = False
